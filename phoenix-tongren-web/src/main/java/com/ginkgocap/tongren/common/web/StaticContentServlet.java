...
public class StaticContentServlet extends HttpServlet {
	private final static Logger logger = LoggerFactory.getLogger(StaticContentServlet.class);
	...
	public void init() throws ServletException {
		...
		WebConstants.ONLINE_VIEW_PATH=this.getServletContext().getRealPath("/onlinefiles");
		File file=new File(WebConstants.ONLINE_VIEW_PATH);
		logger.info("on line path is "+WebConstants.ONLINE_VIEW_PATH);
		if(file.exists()==false){
			logger.warn("will create dir  "+WebConstants.ONLINE_VIEW_PATH);
			file.mkdirs();
			throw new ServletException("path is not exits "+WebConstants.ONLINE_VIEW_PATH);
		}else if(file.isDirectory()==false){
			throw new ServletException("path is not an dir: "+WebConstants.ONLINE_VIEW_PATH);
		}
	}
